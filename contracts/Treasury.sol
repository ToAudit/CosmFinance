...
contract Treasury is Ownable {
    ...
    constructor(
        ...
        isLiquidityToken[_USDTCSM] = true;
    ...
    /**
        @notice allow approved address to deposit an asset for CSM
        @param _amount uint
    ...
    function deposit(
        ...
        // mint CSM needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(CSM).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn CSM for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint _amount, address _token) external {
        ...
        uint value = valueOf(_token, _amount);
        ICSMERC20(CSM).burnFrom(msg.sender, value);
    ...
    /**
        @notice returns CSM valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf(
        ...
        if (isReserveToken[_token]) {
            // convert amount to match CSM decimals
            value_ = _amount.mul(10 ** IERC20(CSM).decimals()).div(
