...
contract BondDepositoryDai is Ownable {
    ...
    address public immutable CSM; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints CSM when receives principle
    address public immutable DAO; // receives profit share from bond
    address public immutable sCSM; // token sCSM
    address public turbine; // receives csm from turbine bond
    address public community; // receives csm from invite bond
    ...
    struct Bond {
        ...
        uint payout; // CSM remaining to be paid
   ...
   */
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 CSM ( underflow protection )
        ...
        /**
                ...
                approved and
                deposited into the treasury, returning (_amount - profit) CSM
             */
        ...
        if (fee != 0) {
            ...
            IERC20(CSM).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            if (_invite) {
                ...
                IERC20(CSM).approve(address(turbine), _amount);
                ITurbine(turbine).receiveTurbine(_recipient, _amount);
            } else {
                // if user does not want to stake
                IERC20(CSM).transfer(_recipient, _amount);
            }
            // IERC20(CSM).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(CSM).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(CSM).approve(staking, _amount);
   ...
   */
    function maxPayout() public view returns (uint) {
        return IERC20(CSM).totalSupply().mul(terms.maxPayout).div(100000);
    }
   ...
   */
    ...
    /**
     *  @notice calculate current ratio of debt to CSM supply
   *  @return debtRatio_ uint
   */
    function debtRatio() public view returns (uint debtRatio_) {
        uint supply = IERC20(CSM).totalSupply();
   ...
   */
    ...
    /**
     *  @notice calculate amount of CSM available for claim by depositor
   *  @param _depositor address
   *  @param _id uint bond id
   *  @param _invite bool is invite bond
   ...
   */
    ...
    /**
     * @notice cllculate amount of address staked sCSM
   ...
   */
    function getStakedAmount(
        ...
        uint sCSMBal = IsCSM(sCSM).balanceOf(_address);
        IStaking.Claim memory info = IStaking(staking).warmupInfo(_address);
        uint gonsBal = IsCSM(sCSM).balanceForGons(info.gons);
        _stakedAmount = sCSMBal.add(gonsBal);
    }

    /* ======= AUXILLIARY ======= */

    /**
     *  @notice allow anyone to send lost tokens (excluding principle or CSM) to the DAO
   *  @return bool
   */
